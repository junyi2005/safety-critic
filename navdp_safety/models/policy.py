...
class NavDP_Policy_DPT(nn.Module):
    def __init__(self,
                 ...
                 device=None):
        super().__init__()
        # Detect the device automatically (prefer CUDA, fall back to CPU).
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') if device is None else torch.device(device)
        print(f"[NavDP_Policy_DPT] using device: {self.device}")
        ...
        # Imported lazily so the safety critic can be used without diffusers;
        # it is only needed by the diffusion generator.
    ...
    def predict_pointgoal_action(self, goal_point, input_images, input_depths, sample_num=16):
        with torch.no_grad():
            ...
            all_trajectory = torch.cumsum(naction / 8.0, dim=1)
